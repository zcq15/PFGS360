...
parser.add_argument("--datadir", "-d", dest="datadir", default="./datasets/Ricoh360", type=str)
...
parser.add_argument("--dataparser", dest="dataparser", default="odgs-dataparser", type=str)
...
if os.path.exists(join(opts.datadir, "images")):
...
opts.experiment = f"ODGS-{os.path.basename(opts.datadir)}"
dataset = opts.datadir
outputs = opts.savedir
method = OrderedDict(
    [
        ("--experiment-name", opts.experiment),
...
def pipeline(scenes, args):
    ...
    if "evaluate" in opts.stages:
        ...
        keys = ["psnr", "ssim", "lpips", "gaussians", "RPE_t", "RPE_r", "ATE"]
